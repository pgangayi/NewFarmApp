...
async function updateInventoryBulk(env, inventoryData, farmId, userId) {
  ...
  const existingItem = await env.DB.prepare(
    `
    SELECT id, qty FROM inventory_items 
    WHERE farm_id = ? AND name = ?
  `
  )
    .bind(farmId, item_name)
    .all();
  ...
  if (action === "add") {
    if (existingItem.length > 0) {
      ...
      await env.DB.prepare(
        ...
        SET qty = qty + ?, unit_cost = ?, supplier = ?, category = ?, updated_at = CURRENT_TIMESTAMP
        WHERE id = ?
      `
      )
        .bind(quantity, unit_cost, supplier, category, existingItem[0].id)
        .run();
    } else {
      // Create new item
      await env.DB.prepare(
        ...
        INSERT INTO inventory_items (
          farm_id, name, qty, unit_cost, supplier, category, created_at
        ) VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
      `
      )
        .bind(farmId, item_name, quantity, unit_cost, supplier, category)
        .run();
    }
  ...
  await env.DB.prepare(
    ...
    INSERT INTO inventory_transactions (
      farm_id, inventory_item_id, qty_delta, unit, 
      reason_type, reference_type, created_at
    ) VALUES (?, ?, ?, ?, 'bulk_operation', ?, CURRENT_TIMESTAMP)
  `
  )
    .bind(
      farmId,
      existingItem.length > 0 ? existingItem[0].id : null,
...
async function createFinancialEntryBulk(env, financialData, farmId, userId) {
  ...
  const result = await env.DB.prepare(
    ...
    INSERT INTO finance_entries (
      farm_id, type, amount, description, category,
      entry_date, reference_id, notes, created_by, created_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
  `
  )
    .bind(
