...
export function SoilHealthMonitor({ farmId }: SoilHealthMonitorProps) {
  ...
  const getPHStatus = (phLevel: number) => {
    if (phLevel >= 6.0 && phLevel <= 7.0) {
      ...
      return { status: 'slightly_acidic', color: 'text-yellow-600' };
    } else if (phLevel > 7.0 && phLevel <= 7.5) {
      return { status: 'slightly_alkaline', color: 'text-yellow-600' };
    } else {
      return { status: 'extreme', color: 'text-red-600' };
    }
  };

  const getNutrientStatus = (value: number, type: string) => {
    if (type === 'nitrogen') {
      if (value >= 40) return { status: 'high', color: 'text-green-600' };
      if (value >= 20) return { status: 'adequate', color: 'text-green-500' };
      if (value >= 10) return { status: 'moderate', color: 'text-yellow-600' };
      return { status: 'deficient', color: 'text-red-600' };
    } else if (type === 'phosphorus') {
      if (value >= 30) return { status: 'high', color: 'text-green-600' };
      if (value >= 15) return { status: 'adequate', color: 'text-green-500' };
      if (value >= 5) return { status: 'moderate', color: 'text-yellow-600' };
      return { status: 'deficient', color: 'text-red-600' };
    } else if (type === 'potassium') {
      if (value >= 150) return { status: 'high', color: 'text-green-600' };
      if (value >= 100) return { status: 'adequate', color: 'text-green-500' };
      if (value >= 50) return { status: 'moderate', color: 'text-yellow-600' };
      return { status: 'deficient', color: 'text-red-600' };
    }
    ...
    return (
      <div className="border rounded-lg p-6 bg-white shadow">
        <div className="flex items-center justify-center text-red-600">
          ...
          Error loading soil health data: {error.message}
        </div>
      </div>
    );
  }
