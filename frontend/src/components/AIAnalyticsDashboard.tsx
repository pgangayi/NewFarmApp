...
const PerformanceOverviewCards = ({
  ...
  getTrendIcon: (t: string) => JSX.Element;
  getScoreGradient: (s: number) => string;
}) => (
  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
    <Card className="border-0 shadow-lg bg-linear-to-br from-white to-blue-50">
      ...
      <CardContent>
        <div className={`text-3xl font-bold ${getScoreColor(data.summary.overall_score)}`}>
          {data.summary.overall_score}/100
        </div>
        <div className="flex items-center gap-1 mt-2">
          {getTrendIcon(data.summary.performance_trend)}
          <span className="text-sm text-gray-600">{data.summary.performance_trend}</span>
        </div>
        <div className="w-full bg-gray-200 rounded-full h-2 mt-3">
          <div
            className={`bg-linear-to-r ${getScoreGradient(data.summary.overall_score)} h-2 rounded-full transition-all duration-500`}
    ...
    <Card className="border-0 shadow-lg bg-linear-to-br from-white to-green-50">
      ...
      <CardContent>
        <div className={`text-3xl font-bold ${getScoreColor(data.summary.efficiency_rating)}`}>
          {data.summary.efficiency_rating}/100
        </div>
        <div className="flex items-center gap-1 mt-2">
        ...
        <div className="w-full bg-gray-200 rounded-full h-2 mt-3">
          <div
            className={`bg-linear-to-r ${getScoreGradient(data.summary.efficiency_rating)} h-2 rounded-full transition-all duration-500`}
            style={{ width: `${data.summary.efficiency_rating}%` }}
          ></div>
        </div>
      </CardContent>
    </Card>
    ...
    <Card className="border-0 shadow-lg bg-linear-to-br from-white to-purple-50">
      ...
      <CardContent>
        <div className={`text-3xl font-bold ${getScoreColor(data.summary.sustainability_score)}`}>
          {data.summary.sustainability_score}/100
        </div>
        <div className="flex items-center gap-1 mt-2">
        ...
        <div className="w-full bg-gray-200 rounded-full h-2 mt-3">
          <div
            className={`bg-linear-to-r ${getScoreGradient(data.summary.sustainability_score)} h-2 rounded-full transition-all duration-500`}
            style={{ width: `${data.summary.sustainability_score}%` }}
          ></div>
        </div>
      </CardContent>
    </Card>
...
const ModulePerformanceSection = ({
  data,
  getScoreColor,
}: {
  data: ComprehensiveAnalytics;
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span className="font-medium">{data.modules.animals.performance_score || 0}/100</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600">Health Status</span>
              <span className="font-medium">
                {(data.modules.animals.overview as ModuleOverviewObject)?.health_status ||
                  'Unknown'}
              </span>
            </div>
            <div className="flex justify-between items-center">
              ...
              <span className="font-medium">
                {(data.modules.animals.overview as ModuleOverviewObject)?.total_animals || 0}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600">Production Rate</span>
              <span className="font-medium">
                {(data.modules.animals.overview as ModuleOverviewObject)?.production_rate || 0}%
              </span>
            </div>
          </div>
        </CardContent>
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span
                className={`font-bold ${getScoreColor(data.modules.crops.performance_score || 0)}`}
              >
                {data.modules.crops.performance_score || 0}/100
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600">Mature Crops</span>
              <span className="font-medium">
                {(data.modules.crops.overview as ModuleOverviewArrayItem[])?.reduce(
                  (sum: number, c) => sum + (c.mature_crops || 0),
                  0
                ) || 0}
              </span>
            </div>
            <div className="flex justify-between items-center">
              ...
              <span className="font-medium">
                {Math.round(
                  (data.modules.crops.yield_performance?.reduce(
                    (sum: number, y) => sum + (y.yield_efficiency || 0),
                    0
                  ) || 0) / (data.modules.crops.yield_performance?.length || 1)
                ) || 0}
                %
              </span>
            </div>
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span
                className={`font-bold ${getScoreColor(data.modules.finance.performance_score || 0)}`}
              >
                {data.modules.finance.performance_score || 0}/100
              </span>
            </div>
            <div className="flex justify-between items-center">
              ...
              <span
                className={`font-medium ${
                  ((data.modules.finance.overview as ModuleOverviewObject)?.net_profit || 0) >= 0
                    ? 'text-green-600'
                    : 'text-red-600'
                }`}
              >
                $
                {(
                  (data.modules.finance.overview as ModuleOverviewObject)?.net_profit || 0
                ).toLocaleString()}
              </span>
            </div>
            <div className="flex justify-between items-center">
              ...
              <span className="font-medium">
                $
                {(
                  (data.modules.finance.overview as ModuleOverviewObject)?.tax_deductible_amount ||
                  0
                ).toLocaleString()}
              </span>
            </div>
...
const AIRecommendations = ({ data }: { data: ComprehensiveAnalytics }) => (
  <Card className="border-0 shadow-lg bg-linear-to-br from-white to-purple-50">
    <CardHeader>
      <CardTitle className="flex items-center gap-2">
...
const CrossModuleInsights = ({
  ...
  <Card className="border-0 shadow-lg">
    <CardHeader>
      ...
      <CardDescription>Advanced insights combining data from all farm modules</CardDescription>
    </CardHeader>
    <CardContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center p-4 bg-blue-50 rounded-lg">
            ...
            <div className="text-lg font-bold">
              {Math.round(
                (data.modules.weather?.overview as ModuleOverviewObject)?.avg_temperature || 0
              )}
              °C
            </div>
          ...
          <div className="text-center p-4 bg-green-50 rounded-lg">
            ...
            <div className="text-lg font-bold">
              {Math.round(
                (data.modules.weather?.overview as ModuleOverviewObject)?.total_precipitation || 0
              )}
              mm
            </div>
          ...
          <div className="text-center p-4 bg-purple-50 rounded-lg">
            ...
            <div className="text-lg font-bold">
              {Math.round(
                (data.modules.weather?.overview as ModuleOverviewObject)?.avg_humidity || 0
              )}
              %
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="text-center p-3 bg-gray-50 rounded-lg">
            ...
            <div className="text-sm font-bold">
              {data.modules.inventory?.performance_score || 0}/100
            </div>
          ...
          <div className="text-center p-3 bg-gray-50 rounded-lg">
            ...
            <div className="text-sm font-bold">
              {data.modules.tasks?.performance_score || 0}/100
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <div className="flex items-center gap-4 mt-3">
              <Badge className="bg-linear-to-r from-purple-100 to-blue-100 text-purple-800 border-purple-200">
                🤖 AI-Powered
              </Badge>
              <Badge className="bg-linear-to-r from-green-100 to-emerald-100 text-green-800 border-green-200">
                📊 Real-time
              </Badge>
              <div className="flex items-center gap-2">
                <Button
        ...
        {viewMode === 'comprehensive' && comprehensiveData && (
          <div className="space-y-8">
            ...
            />
            <ModulePerformanceSection data={comprehensiveData} getScoreColor={getScoreColor} />
            <AIRecommendations data={comprehensiveData} />
            <CrossModuleInsights
              data={comprehensiveData}
              overallScore={comprehensiveData.summary.overall_score}
            />
          </div>
        )}
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {Object.entries(optimizationData).map(([key, _value]) => (
                <Card
                  key={key}
                  className="border-0 shadow-lg bg-linear-to-br from-white to-green-50"
                >
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
