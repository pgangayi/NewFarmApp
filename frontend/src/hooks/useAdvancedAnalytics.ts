...
export function useAdvancedAnalytics(farmId: number) {
  ...
  const [selectedCategories, setSelectedCategories] = useState<string[]>(['financial', 'operational', 'production']);
  ...
  const trendAnalysis = useMemo(() => {
    ...
    return metrics.reduce((acc, metric) => {
      const trend = {
        ...
        significance: metric.trendPercentage > 10 ? 'high' : metric.trendPercentage > 5 ? 'medium' : 'low'
      };
      acc[metric.id] = trend;
      return acc;
    }, {} as Record<string, any>);
  ...
  return {
    ...
    // Loading states
    isLoading: metricsLoading || insightsLoading || predictiveLoading || performanceLoading || benchmarksLoading,
    error: metricsError,
...
export function useRealTimeAnalytics(farmId: number) {
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`${window.location.protocol === 'https:' ? 'wss:' : 'ws:'}//${window.location.host}/api/analytics/realtime?farm_id=${farmId}`);
