...
export function useMobileResponsive(config: Partial<ResponsiveConfig> = {}) {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      const breakpoint = (Object.entries(mergedConfig.breakpoints) as [Breakpoint, number][])
        .reverse()
  ...
  const getResponsiveValue = useCallback(<T>(values: ResponsiveValue<T> | T): T => {
    ...
    return responsiveValues.xs ?? responsiveValues.sm ?? responsiveValues.md ?? 
           responsiveValues.lg ?? responsiveValues.xl ?? responsiveValues['2xl']! as T;
  }, [currentBreakpoint]);

  // Check if current breakpoint matches or is above
  const isBreakpoint = useCallback((breakpoint: Breakpoint) => {
    const currentIndex = (Object.keys(mergedConfig.breakpoints) as Breakpoint[]).indexOf(currentBreakpoint);
    const targetIndex = (Object.keys(mergedConfig.breakpoints) as Breakpoint[]).indexOf(breakpoint);
  ...
  const getResponsiveClasses = useCallback((classes: ResponsiveValue<string>) => {
    return getResponsiveValue(classes);
  }, [getResponsiveValue]);

  // Get responsive styles
  const getResponsiveStyles = useCallback((styles: ResponsiveValue<React.CSSProperties>) => {
  ...
  // Get font size
  const getFontSize = useCallback((size: keyof typeof defaultConfig.fontSize) => {
...
export function useResponsiveGrid() {
  ...
  const getGridColumns = useCallback((columns: ResponsiveValue<number>) => {
    return getResponsiveValue(columns);
  }, [getResponsiveValue]);
  
  const getGridGap = useCallback((gap: ResponsiveValue<number>) => {
