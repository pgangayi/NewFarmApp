...
const sqlFile = path.join(
  ...
  "..",
  "migrations",
  "rollback-token-revocation-schema.sql"
);
...
statements.forEach((stmt, i) => {
  console.log(
    `${i + 1}. ${stmt.substring(0, 80)}${stmt.length > 80 ? "..." : ""}`
  );
});
...
console.log(
  "wrangler d1 execute your-db-name --file=migrations/rollback-token-revocation-schema.sql"
);
