-- Migration 0018: Missing Tables and Schema Fixes
-- Addresses critical schema mismatches found in application code analysis
...
-- ============================================================================
-- FARM ANALYTICS AND OPERATIONS TABLES
-- ============================================================================

-- Farm Statistics (Analytics Data)
...
-- Crop Activities (Detailed tracking)
...
-- Crop Observations (Health and monitoring)
...
-- ============================================================================
-- ANIMAL MANAGEMENT TABLES (Enhanced)
-- ============================================================================
...
ALTER TABLE farms ADD COLUMN farm_type TEXT DEFAULT 'conventional';
ALTER TABLE farms ADD COLUMN certification_status TEXT DEFAULT 'none';
ALTER TABLE farms ADD COLUMN environmental_compliance TEXT DEFAULT 'in_progress';
ALTER TABLE farms ADD COLUMN total_acres REAL DEFAULT 0;
ALTER TABLE farms ADD COLUMN operational_start_date DATE;
ALTER TABLE farms ADD COLUMN management_structure TEXT;
ALTER TABLE farms ADD COLUMN seasonal_staff INTEGER DEFAULT 0;
ALTER TABLE farms ADD COLUMN annual_budget REAL DEFAULT 0;

-- Add missing columns to fields table  
ALTER TABLE fields ADD COLUMN area_sqm REAL; -- Add square meters for consistency
ALTER TABLE fields ADD COLUMN soil_type TEXT;
ALTER TABLE fields ADD COLUMN irrigation_system TEXT;
ALTER TABLE fields ADD COLUMN last_tested_date DATE;

-- Add missing columns to inventory_items table
ALTER TABLE inventory_items ADD COLUMN category TEXT;
ALTER TABLE inventory_items ADD COLUMN supplier_info TEXT; -- JSON
...
-- Insert default farm statistics record for existing farms
INSERT INTO farm_statistics (farm_id, report_date)
SELECT id, date('now') 
FROM farms 
WHERE NOT EXISTS (
    SELECT 1 FROM farm_statistics fs WHERE fs.farm_id = farms.id
);

-- Create default inventory supplier categories
